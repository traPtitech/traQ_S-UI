...
useMittListener(wsListener, 'QALL_ROOM_STATE_CHANGED', async ({ state }) => {
  purifyRoomData([state]).then(purifiedRooms => (rooms.value = purifiedRooms))
})
...
const purifyRoomData = async (
  data: QallRoomStateChangedEventState[]
): Promise<Rooms> => {
  if (!data) return []
